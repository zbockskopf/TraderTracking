...
public class PagerSettings<SelectionType>: ObservableObject where SelectionType: Hashable {
    ...
    private func recalculateTransition() {
        let indexAndPercentage = round(-contentOffset / width)
        let percentage = (indexAndPercentage + 1).truncatingRemainder(dividingBy: 1)
//        print(indexAndPercentage, floor(indexAndPercentage))
        var lowIndex = 0
        guard (!indexAndPercentage.isNaN || !indexAndPercentage.isInfinite) else {
            lowIndex = Int(floor(indexAndPercentage))
            return
        }
