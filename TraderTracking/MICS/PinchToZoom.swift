...
extension View {
    @ViewBuilder
    func pinchZoom(_ dimsBackground: Bool = true, onZoomThresholdReached: @escaping () -> Void) -> some View {
        PinchZoomHelper(dimsBackground: dimsBackground, onZoomThresholdReached: onZoomThresholdReached) {
            self
        }
    }
}
...
fileprivate struct PinchZoomHelper<Content: View>: View {
    ...
    var onZoomThresholdReached: () -> Void
    ...
    var body: some View {
        content
            ...
            .overlay(GestureOverlay(config: $config, onZoomThresholdReached: onZoomThresholdReached))
            .overlay {
                GeometryReader {
                    ...
                    Color.clear
                        ...
                        .onChange(of: config) { oldValue, newValue in
                            if config.isGestureActive && !containerData.isResetting {
                                ...
                                /// Trigger loading of full-resolution image when zoom exceeds threshold
                                if config.zoom > 1.5 {
...
fileprivate struct GestureOverlay: UIViewRepresentable {
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(config: $config, onZoomThresholdReached: onZoomThresholdReached)
    }
    ...
    class Coordinator: NSObject, UIGestureRecognizerDelegate {
        ...
        init(config: Binding<Config>, onZoomThresholdReached: @escaping () -> Void) {
            self._config = config
            self.onZoomThresholdReached = onZoomThresholdReached
        }
