        ...
        let config = Realm.Configuration(schemaVersion: 28,
                                                 migrationBlock: { migration, oldSchemaVersion in
                                                    ...
                                                    if (oldSchemaVersion <= 13){
                                                        migration.enumerateObjects(ofType: News.className()) { (old, new) in
                                                            new!["isSameTime"] = false
...
//            let account = realm.object(ofType: Account.self, forPrimaryKey: "Main")
    ...
    func updateAccountDailyBalanceAfterImport(orders: [(ImportOrder, [ImportOrder], ImportOrder?, ImportOrder?)]){
...
//            self.addNews(news: tempNews)
    ...
    func addImportOrders(orders: [(ImportOrder, [ImportOrder], ImportOrder?, ImportOrder?)]){
        var ntc = NewTradeController()
        try! realm.write {
            for order in orders {
                var importAccount = accounts.first(where: {$0.name == order.0.account}) ?? {
                    let temp = Account()
                ...
                var tempTrade = Trade()
                tempTrade.symbol = symbols.first(where: {$0.name == order.0.product})
...
//                print(tempTrade.dateEntered, "\n")
                ...
                if order.1.count == 0 {
                    tempTrade.dateExited = myFormatter.convertImportDate(date: order.2?.fillTime ?? "")
                }else{
                    tempTrade.dateExited = Date()
                }
                tempTrade.exit = order.1.count != 0 ? 0 : ntc.formatDecimal(str: order.2!.avgFillPrice)
                tempTrade.positionSize = ((order.1.count) > 0 ? ntc.getImportPositionSize(partials: order.1) : Double(order.2?.filledQty ?? ""))!
                tempTrade.positionType = order.0.bs == " Buy" ? .long : .short
...
//                print(order.0.orderId, tempTrade.fees, String(tempTrade.positionSize * tempTrade.symbol!.fees.doubleValue), Decimal128(floatLiteral: (tempTrade.positionSize * tempTrade.symbol!.fees.doubleValue)))
                ...
                tempTrade.partials.append(objectsIn: ntc.convertImportPartils(orders: order.1))
                ...
                tempTrade.stopLoss = order.3 != nil ? try! Decimal128(string: order.3!.stopPrice) : tempTrade.loss ?? false ? tempTrade.exit : nil
...
//            }
    ...
    func addDailyReview(newDailyReview: DailyReview){
        try! realm.write{
            realm.add(newDailyReview)
        }
    }
