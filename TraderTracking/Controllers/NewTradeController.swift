...
class NewTradeController: ObservableObject {
    ...
    func getImportProfitLoss(order: (ImportOrder, [ImportOrder], ImportOrder?, ImportOrder?), symbol: Symbol) -> Decimal128 {
        if order.1.count > 0{
            ...
            if order.2?.avgFillPrice != order.0.avgFillPrice{
                if order.0.bs == " Buy" {
                    return ((Decimal128(floatLiteral: (Double(order.2!.avgFillPrice)! - Double(order.0.avgFillPrice)!)) * 4) * symbol.tickValue) * Decimal128(floatLiteral: Double( order.2!.filledQty)!)
                }else{
                    return ((Decimal128(floatLiteral: (Double(order.0.avgFillPrice)! - Double(order.2!.avgFillPrice)!)) * 4) * symbol.tickValue) * Decimal128(floatLiteral: Double( order.2!.filledQty)!)
                }
            }else{
                return 0.0
            }
